...
public class SwordFollow : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (pmScript.attacking || (pmScript.swordAnimControl && !pmScript.pAttack.isParrying && !PlayerHealth.dead))
        {
            AttackMove();
    ...
    private void Update()
    {
        ...
        {
            if(InputReader.inputs.actions["Attack"].WasPressedThisFrame() && lastParryFail + parryFailCooldown < Time.time)
            {
